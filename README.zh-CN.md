...
Horizon-Aurora 是 Horizon 系列中的全文润色工具。它可以润色英文学术论文，将中文学术文本翻译成英文，也可以直接处理 LaTeX/TeX 项目。

它主要解决两类全文编辑问题：

1. **孤立的局部修改**：单段文字有所改善，但术语、论断强度或适用范围在不同章节间发生偏移。
2. **不完整的最终核查**：只检查了中间版本，后续修复或包含文件没有得到验证。

Aurora 先进行一次整合式改写，再执行保真核查、全局读者通读与主张对齐、定点修复、重点一致性审查和最终核查。它保护科学含义，不评估创新性、不重新设计贡献，也不修复科学论证本身尚不稳定的论文。

1.0.0 版提供分章节指导、独立的原稿与候选项目管理，以及明确的全文／部分覆盖报告。检查器支持导言区和表格保护、自定义宏登记及翻译场景下的数值核查，并说明需要人工复核的范围。

### 模式

| 模式 | 使用场景 |
|---|---|
| **读者导向** | 全文润色，改善段落推进并核对跨章节主张 |
| **严格 token 保留** | 最小差异编辑和更强的机械保真 |
| **翻译加润色** | 将中文学术文本转换为自然英文，但不强化原文含义 |
| **核查** | 比较原文与修订稿，不进行改写 |
| **一致性审查** | 检查术语、符号、数值关系和反复出现的主张 |
| **仅诊断** | 识别高影响的可读性和保真问题，不进行改写 |

## Horizon-Aegis

Horizon-Aegis 是 Horizon 系列中的只读科学审查工具。它在投稿或大修前审查技术论文，核对创新性、方法、报告数值、证据，以及已提供的实现材料。

它主要解决两类常见问题：

1. **方法描述不一致**：文字、公式、设置、结果或代码对应了不同版本的方法。
2. **贡献归因尚未成立**：完整方法有效，但证据未能说明提升来自所声称的贡献，也未证明该贡献的必要性。

Aegis 建立论断—证据矩阵，检验可信的替代解释，并提出最小决定性证据、收窄论断或重构贡献的建议。它只返回审查结果，不修改论文。未提供代码只限制审查范围，本身不构成论文缺陷。

**1.0.0** 为首次公开发布。安装、提示词、报告说明和完整工作流程见 [Aegis 使用指南](skills/horizon-aegis/README.md)。
...
```text
全局理解 → 锁定含义 → 诊断 → 一次整合式改写
    → 保真核查 → 全局读者通读与主张对齐
    → 定点修复 → 重点一致性审查 → 最终核查
```

Aurora 在章节之间保持论文级上下文，并在全部修复完成后核查最终交付文件。全文或部分覆盖状态以实际完成阅读、编辑与验证的材料为准。
