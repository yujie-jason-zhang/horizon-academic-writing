...
# En-length dashes between numeric endpoints are standard range notation in TeX
# (`5--10`, `pp.~12--18`, `0.5--1.0`, `Figs.~3a--3c`), not the parenthetical dash
# punctuation this profile discourages. Exempt them so that a byte-identical
# candidate cannot fail the dash check.
RANGE_ENDPOINT_RE = r"\d+(?:\.\d+)?[A-Za-z]?"
EN_RANGE_DASH_RE = r"(?<!-)--(?!-)|[\u2012\u2013]"
NUMERIC_RANGE_DASH_RE = re.compile(
    rf"(?<![A-Za-z0-9]){RANGE_ENDPOINT_RE}\s*(?P<dash>{EN_RANGE_DASH_RE})\s*"
    rf"{RANGE_ENDPOINT_RE}(?![A-Za-z0-9])"
)
...
TABLE_ENVIRONMENTS = {"tabular", "tabular*", "tabularx", "longtable"}

# Only citation/reference commands can move under --allow-structural-reorder.
# Labels, environments, paths, comments, and all other structure remain ordered.
REORDERABLE_COMMANDS = (
    {name for name in OPAQUE_COMMAND_ARGUMENT_COUNTS if "cite" in name}
    | VARIABLE_PROTECTED_ARGUMENT_COMMANDS
    | {name.lower() for name in REFERENCE_COMMANDS}
    | {"crefrange", "cpagerefrange", "pagerefrange", "refrange", "vrefrange"}
) - {"label", "nocite"}

# Populated only by explicit --protect-command NAME=N CLI arguments. Unknown
# macro arguments cannot safely be classified as prose or opaque data by name.
CUSTOM_PROTECTED_ARGUMENT_COUNTS: dict[str, int] = {}
...
def collect_opaque_command_argument_spans(
    ...
    for match in TEX_COMMAND_RE.finditer(text):
        ...
        if included_commands is not None and command not in included_commands:
            continue
        argument_count = CUSTOM_PROTECTED_ARGUMENT_COUNTS.get(
            raw_command, OPAQUE_COMMAND_ARGUMENT_COUNTS.get(command)
        )
...
def collect_prohibited_dashes(text: str) -> list[tuple[int, str, str]]:
    ...
    range_spans = [
        match.span("dash") for match in NUMERIC_RANGE_DASH_RE.finditer(masked_text)
    ]
    issues: list[tuple[int, str, str]] = []
    for match in PROHIBITED_DASH_RE.finditer(masked_text):
        if any(
            start <= match.start() and match.end() <= end for start, end in range_spans
        ):
            continue
...
def collect_structural_events(text: str) -> list[StructuralEvent]:
    ...
    while cursor < len(active):
        next_token = STRUCTURAL_TOKEN_RE.search(active, cursor)
        if next_token is None:
            break
        command_start = next_token.start()
        if is_escaped(active, command_start):
            cursor = command_start + 1
            continue
        in_table = any(name in TABLE_ENVIRONMENTS for name in environment_stack)
        if active[command_start] == "&":
            if in_table:
                indexed_events.append(
                    (
                        command_start,
        ...
        if command_match is None:
            ...
            signature = ("control-symbol", source)
            if in_table and source == "\\\\":
                starred = end < len(active) and active[end] == "*"
                if starred:
                    end += 1
                option_start = skip_tex_whitespace(active, end)
                option = ""
                if option_start < len(active) and active[option_start] == "[":
                    option_end = find_balanced_end(active, option_start, "[", "]")
                    if option_end is not None:
                        option = active[option_start:option_end]
                        end = option_end
                signature = ("table-row", starred, option)
                source = text[command_start:end]
            indexed_events.append(
                (
                    command_start,
        ...
        argument_count = CUSTOM_PROTECTED_ARGUMENT_COUNTS.get(
            command, CASE_SENSITIVE_PROTECTED_ARGUMENT_COUNTS.get(command)
        )
        if argument_count is None:
...
def select_events_for_signatures(
    events: list[StructuralEvent], signature_counts: Counter[str]
) -> list[StructuralEvent]:
    remaining = Counter(signature_counts)
    selected: list[StructuralEvent] = []
    for event in events:
        if remaining.get(event.signature, 0) > 0:
            selected.append(event)
            remaining[event.signature] -= 1
    return selected


def structural_event_multiset_delta(
    original: list[StructuralEvent], polished: list[StructuralEvent]
) -> tuple[list[StructuralEvent], list[StructuralEvent]]:
    """Compare an explicitly selected set of movable events by inventory."""
    original_counts = Counter(event.signature for event in original)
    polished_counts = Counter(event.signature for event in polished)
    missing = select_events_for_signatures(original, original_counts - polished_counts)
    added = select_events_for_signatures(polished, polished_counts - original_counts)
    return missing, added


def is_reorderable_event(event: StructuralEvent) -> bool:
    return (
        event.signature[0] == "protected-command"
        and str(event.signature[1]).lower() in REORDERABLE_COMMANDS
    )


def document_preamble(text: str) -> str | None:
    """Return exact source before a top-level active document environment."""
    active = mask_inactive_tex(text, mask_math=True)
    depth = 0
    for index, char in enumerate(active):
        if is_escaped(active, index):
            continue
        if char == "\\" and depth == 0:
            marker = ENVIRONMENT_MARKER_RE.match(active, index)
            if marker and marker.group("action") == "begin" and marker.group("name") == "document":
                return text[:index]
        elif char == "{":
            depth += 1
        elif char == "}":
...
def check_preamble_preservation(original_text: str, polished_text: str) -> bool:
    original = document_preamble(original_text)
    polished = document_preamble(polished_text)
    passed = original == polished
    note = " (fragments without a document preamble)" if original is None and polished is None else ""
    print(f"Preamble source: {'PASS' if passed else 'FAIL'}{note}")
    if not passed:
        print("  Preserve the preamble byte-for-byte; compare explicitly authorized edits against an approved baseline.")
    return passed


def check_project_preambles(
    original: dict[str, str | None], polished: dict[str, str | None], limit: int,
) -> bool:
    """Check subfile preambles as well as the expanded root's preamble."""
    changed = [
        path for path in sorted(original.keys() | polished.keys())
        if path not in original or path not in polished or original[path] != polished[path]
    ]
    print(f"Project file preambles: {'FAIL' if changed else 'PASS'}")
    for path in changed[:limit]:
        print(f"  Changed preamble or file inventory: {path}")
...
def check_structural_preservation(
    ...
    reordered = False
    if allow_reordering:
        missing, added = structural_event_sequence_delta(
            [event for event in original_events if not is_reorderable_event(event)],
            [event for event in polished_events if not is_reorderable_event(event)],
        )
        missing_movable, added_movable = structural_event_multiset_delta(
            [event for event in original_events if is_reorderable_event(event)],
            [event for event in polished_events if is_reorderable_event(event)],
        )
        missing.extend(missing_movable)
        added.extend(added_movable)
        reordered = (
            not missing
            and not added
            and [event.signature for event in original_events]
            != [event.signature for event in polished_events]
        )
    else:
        missing, added = structural_event_sequence_delta(
            original_events, polished_events
        )
    ...
    if reordered:
        print(
            "  Citation/reference order changed; their inventory and other structural event order are preserved. "
            "This does not verify semantic attachment; "
            "manually confirm that each citation/reference remains attached to the intended claim."
        )
...
def collect_numeric_occurrences(text: str, *, translation: bool = False) -> list[NumericOccurrence]:
    ...
    def is_numeric_character(char: str) -> bool:
        if not char.isnumeric():
            return False
        # Han characters can be numerals or parts of ordinary words (e.g.
        # 参见). In translation mode these require semantic review, like
        # spelled-out English numbers; do not pretend to tokenize Chinese.
        return not (
            translation
            and unicodedata.name(char, "").startswith(
                ("CJK UNIFIED IDEOGRAPH", "CJK COMPATIBILITY IDEOGRAPH", "IDEOGRAPHIC")
            )
        )
    ...
    def source_contains_number(source: str) -> bool:
        return (
            any(is_numeric_character(char) for char in source)
            or (
                not translation
                and (
                    TEXTUAL_NUMBER_PATTERN.search(source) is not None
    ...
    cursor = 0
    while cursor < len(text):
        if covered[cursor] or not is_numeric_character(text[cursor]):
            cursor += 1
            continue
        number_start = cursor
        while (
            ...
            and is_numeric_character(text[cursor])
        ):
            cursor += 1
        number_end = cursor
    ...
    for match in COMMENT_SPLICED_MARKER_PATTERN.finditer(text):
        ...
        if translation and TEXTUAL_NUMBER_PATTERN.fullmatch(raw_value):
            continue
...
def numeric_multiset_delta(
    original: list[NumericOccurrence], polished: list[NumericOccurrence]
) -> tuple[list[NumericOccurrence], list[NumericOccurrence]]:
    """Compare recognized numeric occurrences without requiring document order."""
    original_counts = Counter(occurrence.signature for occurrence in original)
    polished_counts = Counter(occurrence.signature for occurrence in polished)
    missing_counts = original_counts - polished_counts
    added_counts = polished_counts - original_counts

    missing: list[NumericOccurrence] = []
    added: list[NumericOccurrence] = []

    remaining = missing_counts.copy()
    for occurrence in original:
        if remaining[occurrence.signature] > 0:
            missing.append(occurrence)
            remaining[occurrence.signature] -= 1

    remaining = added_counts.copy()
    for occurrence in polished:
        if remaining[occurrence.signature] > 0:
...
def check_numeric_preservation(
    original_text: str, polished_text: str, limit: int, allow_reordering: bool = False,
    translation: bool = False,
) -> bool:
    """Require exact recognized numeric content; optionally ignore document order."""
    original = collect_numeric_occurrences(original_text, translation=translation)
    polished = collect_numeric_occurrences(polished_text, translation=translation)
    if allow_reordering:
        missing, added = numeric_multiset_delta(original, polished)
        label = "Recognized numeric/unit/sign/marker multiset"
    else:
        missing, added = numeric_sequence_delta(original, polished)
        label = "Recognized numeric/unit/sign/marker sequence"
    passed = not missing and not added
    print(f"{label}: {'PASS' if passed else 'FAIL'}")
    if allow_reordering:
        print(
            "Numeric occurrence order: NOT ENFORCED (reader-oriented mode; "
            "manually verify metric/condition/value bindings)."
        )
...
def report_placeholder_reference_advisory(polished_text: str, limit: int) -> None:
    placeholders = collect_placeholder_reference_keys(polished_text)
    if not placeholders:
        return

    print("Placeholder label/reference keys: ADVISORY (does not affect preservation)")
    print("  Short keys may be intentional. Preserve existing keys; review naming separately only if requested.")
...
def report_hardcoded_numbering_advisory(polished_text: str, limit: int) -> None:
    ...
    print("Hard-coded display numbering: ADVISORY (does not affect preservation)")
    print(
        "  Preserve existing numbering during polishing. Review generated references separately if requested; "
        "this style advisory is not evidence of an introduced preservation error."
    )
...
def report_reference_name_style_advisory(original_text: str, polished_text: str, limit: int) -> None:
    ...
    print("Reference-name style: ADVISORY (does not affect preservation)")
...
def report_dash_style_advisory(polished_text: str, limit: int) -> None:
    """Report dash-heavy prose as an optional style advisory.

    Dash style is intentionally excluded from preservation pass/fail because it does
    not indicate whether protected scientific or TeX content was preserved.
    """
    issues = collect_prohibited_dashes(polished_text)
    if not issues:
        print("Dash style advisory: no em-length prose dashes detected")
        return

    print("Dash style advisory: review em-length prose dashes if the target style disfavors them")
...
def read_text(path: Path) -> str:
    # Preserve line endings so byte-identical preamble protection also detects
    # CRLF/LF changes. UTF-8 decoding is otherwise lossless.
    with path.open(encoding="utf-8", newline="") as stream:
        return stream.read()


def parse_protected_commands(values: list[str]) -> dict[str, int]:
    commands: dict[str, int] = {}
    for value in values:
        match = re.fullmatch(r"([A-Za-z@]+)=([0-9])", value)
        if match is None:
            raise ValueError("--protect-command requires NAME=N, with a command name and 0–9 required brace arguments")
        name, count_text = match.groups()
        if (
            name.lower() in OPAQUE_COMMAND_ARGUMENT_COUNTS
            or name in CASE_SENSITIVE_PROTECTED_ARGUMENT_COUNTS
            or "cite" in name.lower()
        ):
            raise ValueError(f"\\{name} already has built-in protection; its argument handling cannot be overridden")
        count = int(count_text)
        if name in commands and commands[name] != count:
            raise ValueError(f"conflicting argument counts for \\{name}")
        commands[name] = count
    return commands
...
def load_tex_project(
    root_path: Path, preambles: dict[str, str | None] | None = None,
) -> tuple[str, list[ProjectLoadIssue]]:
    ...
    def expand(
        ...
        if preambles is not None:
            preambles[relpath(resolved_path, project_root)] = document_preamble(text)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Reader-oriented polishing mode: preserve recognized numeric values/units/signs as a multiset rather than "
            "requiring document order. Use only after manual verification that moved numerical statements retain the "
            "same metric, condition, dataset, and comparison."
        ),
    )
    parser.add_argument(
        "--style-check",
        action="store_true",
        help=(
            "Run optional advisory-only style checks. Currently this surfaces em-length dash punctuation in prose. "
            "Style advisories never affect preservation pass/fail."
        ),
    )
    parser.add_argument(
        "--allow-structural-reorder",
        action="store_true",
        help=(
            "Compare supported citation and cross-reference commands as a multiset; keep all other structural "
            "events ordered. Use only when sentences or clauses containing \\cite/\\ref were deliberately reordered during "
            "reader-oriented polishing. Counts and keys remain exact, but semantic claim-citation attachment is not "
            "verified and must be checked manually."
        ),
    )
    parser.add_argument(
        "--approved-symbol-map",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "For Chinese-to-English comparison, exclude Han numerals and spelled-out/contextual Roman number "
            "words from token equality. Digits, recognized units/signs, math, and protected TeX remain strict. "
            "Manually verify ALL word-based quantities and translated unit spellings; this option does not "
            "normalize unit names or prove numerical equivalence across languages."
        ),
    )
    parser.add_argument(
        "--protect-command",
        ...
        metavar="NAME=N",
        help=(
            "Protect a custom command's optional arguments and first N required brace arguments (0–9). "
            "Repeat for additional commands, e.g. --protect-command artifact=1. Unregistered custom arguments "
            "require manual source comparison; this is not a general TeX macro interpreter."
        ),
    )
    parser.add_argument(
        "--project",
    ...
    if args.reader_oriented and args.approved_symbol_map:
        parser.error("--reader-oriented cannot be combined with --approved-symbol-map")
    if args.translation and args.approved_symbol_map:
        parser.error("--translation cannot be combined with --approved-symbol-map")
    if args.allow_structural_reorder and args.approved_symbol_map:
        parser.error(
            "--allow-structural-reorder cannot be combined with --approved-symbol-map"
        )
    try:
        approved_symbol_maps = parse_approved_symbol_maps(args.approved_symbol_map)
        custom_commands = parse_protected_commands(args.protect_command)
    except ValueError as exc:
        parser.error(str(exc))
    CUSTOM_PROTECTED_ARGUMENT_COUNTS.clear()
    CUSTOM_PROTECTED_ARGUMENT_COUNTS.update(custom_commands)

    unchecked_includes = False
    original_preambles: dict[str, str | None] = {}
    polished_preambles: dict[str, str | None] = {}
    try:
        if args.project:
            original_text, original_project_issues = load_tex_project(args.original, original_preambles)
            polished_text, polished_project_issues = load_tex_project(args.polished, polished_preambles)
        else:
            original_text = read_text(args.original)
            polished_text = read_text(args.polished)
            original_project_issues = []
            polished_project_issues = []
            for source, path in ((original_text, args.original), (polished_text, args.polished)):
                includes, issues = collect_static_includes(source, path)
                if includes or issues:
                    unchecked_includes = True
    except (OSError, UnicodeError) as exc:
        print(f"Error reading input files: {exc}", file=sys.stderr)
        return 2

    project_pass = not original_project_issues and not polished_project_issues and not unchecked_includes
    if args.project:
    ...
    elif unchecked_includes:
        print("Project coverage: FAIL (includes detected but not traversed)")
        print("  Re-run with --project on root files in separate original and candidate project copies.")

    if custom_commands:
        print("Custom command protection: " + ", ".join(f"\\{name}: {count} required argument(s)" for name, count in sorted(custom_commands.items())))
    print("Manual review required for unregistered custom macro arguments and semantic bindings.")
    if args.translation:
        print(
            "Translation coverage: PARTIAL for quantities. Word-based numbers require manual bilingual review; "
            "digits and recognized units/signs remain strict. Translated unit spellings are not normalized."
        )
    ...
    if args.reader_oriented:
        print(
            "Reader-oriented mode: numeric occurrence order is relaxed, but values/units/signs remain strict."
        )

    if args.allow_structural_reorder:
        print(
            "Structural reorder mode: citation and cross-reference order is relaxed, but keys and counts remain "
            "strict. Semantic claim attachment is not machine-verified; review it manually."
        )

    preamble_pass = check_preamble_preservation(original_text, polished_text)
    if args.project:
        preamble_pass = check_project_preambles(original_preambles, polished_preambles, args.limit) and preamble_pass
    structures_pass = check_structural_preservation(
        original_text, polished_text, args.limit, allow_reordering=args.allow_structural_reorder
    )
    ...
    numbers_pass = check_numeric_preservation(
        original_text, polished_text, args.limit, allow_reordering=args.reader_oriented,
    ...
    if approved_symbol_maps:
        ...
        report_placeholder_reference_advisory(polished_text, args.limit)
        report_hardcoded_numbering_advisory(polished_text, args.limit)
        report_reference_name_style_advisory(
            original_text, polished_text, args.limit
        )
        if args.style_check:
            report_dash_style_advisory(polished_text, args.limit)
